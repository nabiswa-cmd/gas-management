...
def sales_form():
    try:
        ...
        # Fetch gas data for dropdown
        cur.execute("SELECT gas_id, gas_name, empty_cylinders, filled_cylinders FROM gas_table;")
        gases = cur.fetchall()

        # Fetch latest sales (limit 10, most recent first)
        cur.execute("""
            SELECT s.sale_id, g.gas_name, s.amount_paid_cash, s.amount_paid_till, s.sale_time
            FROM sales_table s
...
@app.route('/sales')
def sales():
    return render_template('sales.html')  # Make sure sales.html exists

@app.route('/refill')
def refill():
    return render_template('refill.html')  # Make sure refill.html exists
